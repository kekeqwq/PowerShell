...
# 展开并解析公钥文件路径（避免 UAC 提权切换目录导致相对路径失效）
if ($AuthorizedKeysPath) {
    if ($AuthorizedKeysPath.StartsWith("~")) {
        $AuthorizedKeysPath = Join-Path $HOME $AuthorizedKeysPath.Substring(1).TrimStart('\', '/')
    } elseif (-not [System.IO.Path]::IsPathRooted($AuthorizedKeysPath)) {
        $AuthorizedKeysPath = [System.IO.Path]::GetFullPath((Join-Path (Get-Location) $AuthorizedKeysPath))
    }
} else {
    $defaultKeys = @(
...
if (-not $isAdmin -and -not $SkipAdminCheck) {
    ...
    if ($AuthorizedKeysPath) {
        $argList += "`"$AuthorizedKeysPath`""
    }
    try {
        $p = Start-Process -FilePath $callerExe -ArgumentList $argList -Verb RunAs -PassThru -Wait
        exit $p.ExitCode
    } catch {
        Write-Warning "[-] 无法自动拉起管理员权限窗口（可能在无图形界面的远程/后台终端中运行）。"
        Write-Warning "[-] 请右键以管理员身份运行终端后重新执行脚本。"
        exit 1
    }
}
...
if ($AuthorizedKeysPath -and (Test-Path $AuthorizedKeysPath)) {
    Write-Host "[+] 使用公钥文件: $AuthorizedKeysPath" -ForegroundColor Green
} else {
    Write-Warning "[-] 未指定或未找到公钥文件，稍后需手动配置 ~/.ssh/authorized_keys"
}
...
try {
    ...
    Write-Warning "[-] 自动下载最新 Preview 版失败: $_。将使用现有 PowerShell"
}
...
if ($AuthorizedKeysPath -and (Test-Path $AuthorizedKeysPath)) {
    ...
    $newKeys = Get-Content -Path $AuthorizedKeysPath | Where-Object { $_ -and $_.Trim() }
